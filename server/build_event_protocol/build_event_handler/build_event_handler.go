...
const (
	...
	// gRPC metadata set to true if an invocation stream is a replayed stream.
	invocationReplayContextKey = "x-buildbuddy-invocation-replay"
)
...
func (e *EventChannel) handleEvent(event *pepb.PublishBuildToolEventStreamRequest) error {
	...
	if isStartedEvent(&bazelBuildEvent) {
		...
		err := e.setAuthContextFromBuildStartedEvent(&bazelBuildEvent)
		if err != nil {
			return err
		}
...
func isReplayingInvocation(ctx context.Context) bool {
	val, ok := ctx.Value(invocationReplayContextKey).(string)
	return ok && val == "true"
}

func (e *EventChannel) setAuthContextFromBuildStartedEvent(event *build_event_stream.BuildEvent) error {
	// Don't set auth credentials when replaying an invocation (since an API key
	// generated by one server likely does not work on another server).
	if isReplayingInvocation(e.ctx) {
		return nil
	}
	...
	apiKey := auth.ParseAPIKeyFromString(options)
	if apiKey == "" {
		return nil
	}
